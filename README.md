...
![UPES cover banner](assets/upes-cover-banner.svg)
...
```mermaid
flowchart LR
    A[Current State: Rapid Growth with Fragmented Systems] --> B[Diagnose Strategic Gaps]
    B --> C1[Declining Retention]
    B --> C2[Operational Inefficiency]
    B --> C3[Reputational Risk]
    B --> C4[Leadership Uncertainty]
    C1 --> D[Unified Data & Analytics Platform]
    C2 --> D
    C3 --> E[NLP-Based Voice of Customer Intelligence]
    C4 --> F[Executive Decision Dashboard]
    D --> G[Predictive Retention, Pricing, Forecasting]
    E --> G
    F --> H[Data-Driven Strategic Control]
    G --> I[Trust-Led Profitable Growth]
    H --> I
    I --> J[Sustainable Competitive Advantage]
```

**Image version for Word/PDF use:**  
![Strategic roadmap diagram](assets/strategy-roadmap.svg)
...
```mermaid
flowchart TD
    A[Data Sources] --> A1[Transactions]
    A --> A2[App/Web Clickstream]
    A --> A3[CRM & Customer Service]
    A --> A4[Warehouse & Logistics]
    A --> A5[Reviews / Social Media / Emails]
    ...
    C --> C1[Raw Zone]
    C --> C2[Cleansed Zone]
    C --> C3[Curated Business Zone]
    C --> C4[Feature Store / NLP Corpus]

    C3 --> D1[BI & Dashboard Layer]
    C4 --> D2[ML / NLP Models]

    D2 --> E1[Churn Prediction]
    D2 --> E2[Demand Forecasting]
    D2 --> E3[Delay Risk Prediction]
    D2 --> E4[Sentiment & Complaint Classification]

    D1 --> F[Executive and Functional Dashboards]
    E1 --> G[CRM Campaigns]
    E2 --> H[Inventory Planning]
    E3 --> I[Operations Alerts]
    E4 --> J[Reputation & Service Response]

    K[Governance, Security, Metadata, Data Quality] --> C
    K --> D1
    K --> D2
```

**Image version for Word/PDF use:**  
![Big data architecture diagram](assets/big-data-architecture.svg)
...
```mermaid
flowchart LR
    A[Text Sources] --> A1[Reviews]
    A --> A2[Chat Logs]
    A --> A3[Emails]
    A --> A4[Social Media]
    A --> A5[Call Transcripts]

    A1 --> B[NLP Preprocessing]
    A2 --> B
    A3 --> B
    A4 --> B
    A5 --> B

    B --> C1[Language Detection]
    B --> C2[Cleaning / Normalization]
    B --> C3[Tokenization / Embeddings]
    ...
    D --> D2[Complaint Classification]
    D --> D3[Entity Extraction]
    D --> D4[Topic Detection]
    D --> D5[Urgency / Escalation Scoring]

    D1 --> E[Voice of Customer Intelligence]
    D2 --> E
    D3 --> E
    D4 --> E
    D5 --> E
    ...
    E --> F1[Service Triage]
    E --> F2[Reputation Alerts]
    E --> F3[CXO Insights]
    E --> F4[Root Cause Analysis]
```

**Image version for Word/PDF use:**  
![NLP workflow diagram](assets/nlp-workflow.svg)
...
```mermaid
flowchart TD
    A[Executive Dashboard] --> B[Strategic Health]
    A --> C[Operations]
    A --> D[Customer & Reputation]
    A --> E[Forward-Looking Decision Support]

    B --> B1[Revenue Growth]
    B --> B2[Contribution Margin]
    B --> B3[Repeat Purchase Rate]
    B --> B4[Net Retention]

    C --> C1[On-Time Delivery]
    C --> C2[Stockouts]
    C --> C3[Hub Delay Heatmap]
    C --> C4[Returns and Refund TAT]

    D --> D1[Sentiment Trend]
    D --> D2[Complaint Taxonomy]
    D --> D3[Reputation Alerts]
    D --> D4[Churn Risk Segments]

    E --> E1[Demand Forecast]
    E --> E2[Scenario Analysis]
    E --> E3[Recommended Actions]
    E --> E4[Risk Indicators]
```

**Image version for Word/PDF use:**  
![Executive dashboard diagram](assets/executive-dashboard.svg)
